...
/**UIScreen width*/
#define  AQscreenWidth   [UIScreen mainScreen].bounds.size.width
/**UIScreen height*/
#define  AQscreenHeight  [UIScreen mainScreen].bounds.size.height
// 播放器的几种状态
typedef NS_ENUM(NSInteger, aqPlayerState) {
    AQPlayerStateFailed,     // 播放失败
    AQPlayerStatePlaying,    // 播放中
    AQPlayerStateBuffering,  // 缓冲中
    AQPlayerStateStopped,    // 停止播放
    AQPlayerStatePause       // 暂停播放
};
// 枚举值，包含水平移动方向和垂直移动方向
...
@interface AQPlayerView()<AQPlayerMaskViewDelegate,UIGestureRecognizerDelegate>

/** 播放器的几种状态 */
@property (nonatomic, assign) aqPlayerState    state;
/**控件原始Farme*/
...
/**遮罩*/
@property (nonatomic, strong) AQPlayerMaskView *maskView;
/**轻拍定时器*/
...
@implementation AQPlayerView

#pragma mark - 懒加载
//遮罩
- (AQPlayerMaskView *) maskView{
    if (_maskView == nil){
        _maskView                         = [[AQPlayerMaskView alloc] init];
...
- (void)setState:(aqPlayerState)state{
    _state = state;
    if (state == AQPlayerStateBuffering) {
        [self.maskView.activity starAnimation];
    }else if (state == AQPlayerStateFailed){
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    if ([keyPath isEqualToString:@"status"]) {
        if (self.player.currentItem.status == AVPlayerItemStatusReadyToPlay) {
            ...
            self.state        = AQPlayerStatePlaying;
            self.player.muted = self.mute;
        }
        else if (self.player.currentItem.status == AVPlayerItemStatusFailed) {
            self.state = AQPlayerStateFailed;
        }
        ...
        // 当缓冲好的时候
        if (self.playerItem.isPlaybackLikelyToKeepUp && self.state == AQPlayerStatePlaying){
            self.state = AQPlayerStatePlaying;
        }
    }
}
...
- (void)panDirection:(UIPanGestureRecognizer *)pan {
    ...
    switch (pan.state) {
        case UIGestureRecognizerStateBegan:{ // 开始移动
            ...
            if (x > y) { // 水平移动
                [self aq_progressSliderTouchBegan:nil];
                // 取消隐藏
        ...
        case UIGestureRecognizerStateEnded:{ // 移动停止
            ...
            switch (self.panDirection) {
                case PanDirectionHorizontalMoved:{
                    // 把sumTime滞空，不然会越加越多
                    self.sumTime = 0;
                    [self aq_progressSliderTouchEnded:nil];
                    break;
                }
                case PanDirectionVerticalMoved:{
                    // 垂直移动结束后，把状态改为不再控制音量
...
- (void)fullScreenWithDirection:(UIInterfaceOrientation)direction{
    ...
    if (_isLandscape == YES){
        ...
        if (keyWindow.frame.size.width < keyWindow.frame.size.height) {
            self.frame = CGRectMake(0, 0, AQscreenHeight, AQscreenWidth);
            self.frame = CGRectMake(0, 0, AQscreenWidth, AQscreenHeight);
        }
    }else{
